...
function renderDynamicMapWidget(node: WidgetNode): string {
  ...
  return `<div class="widget widget-dynamic-map" data-widget-id="${node.id}" data-map-places="${placesJson}" data-map-latitude="${latitude}" data-map-longitude="${longitude}" data-map-request-user-location="${String(requestUserLocation)}" data-map-sort-by-distance="${String(sortByDistance)}" data-map-show-open-now="${String(showOpenNow)}" data-map-show-distance="${String(showDistance)}" data-map-default-cta-type="${escapeHtml(defaultCtaType)}" data-map-default-cta-label="${escapeHtml(defaultCtaLabel)}" data-map-accent="${escapeHtml(accent)}" style="${base}">
    ...
    <div style="padding:8px 12px 12px;flex:1;display:grid;grid-template-columns:${cardsOnly || stackedLayout ? '1fr' : mapFirst ? '1.35fr .65fr' : '.65fr 1.35fr'};grid-template-rows:${stackedLayout ? (mapFirst ? '1.15fr .85fr' : '.85fr 1.15fr') : 'none'};gap:10px;min-height:0;">
      ...
      <div data-map-cards style="display:grid;gap:6px;overflow:auto;min-height:0;padding-right:2px;align-content:start;">${places.map((place) => `<div data-map-card data-place-name="${escapeHtml(place.name)}" style="border-radius:10px;background:rgba(255,255,255,.78);border:1px solid ${escapeHtml(accent)}22;padding:8px 9px;display:grid;gap:4px;">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;"><strong style="font-size:12px;line-height:1.15;">${escapeHtml(place.name)}</strong><span data-place-badge style="font-size:9px;border-radius:999px;padding:3px 6px;background:${escapeHtml(accent)}22;color:#0f172a;white-space:nowrap;">${escapeHtml(place.badge || (place.openNow ? 'Open now' : 'Store'))}</span></div>
        <div style="font-size:10px;opacity:.78;line-height:1.2;">${escapeHtml(place.address || `${place.lat.toFixed(3)}, ${place.lng.toFixed(3)}`)}</div>
        <div data-place-meta style="display:flex;gap:6px;flex-wrap:wrap;font-size:10px;">${showOpenNow && place.openNow != null ? `<span data-place-open-now>${place.openNow ? 'Open now' : 'Closed'}</span>` : ''}</div>
        <div style="display:flex;gap:6px;flex-wrap:wrap;">
          <button type="button" data-smx-action="map-place-cta" data-place-url="${escapeHtml(buildPlaceCtaUrl(place, 'waze'))}" style="display:inline-flex;align-items:center;justify-content:center;min-width:42px;height:28px;border-radius:999px;padding:0 10px;color:#fff;font-size:9px;font-weight:800;text-decoration:none;border:none;background:#08d4ff;cursor:pointer;">Waze</button>
          <button type="button" data-smx-action="map-place-cta" data-place-url="${escapeHtml(buildPlaceCtaUrl(place, 'maps'))}" style="display:inline-flex;align-items:center;justify-content:center;min-width:42px;height:28px;border-radius:999px;padding:0 10px;color:#fff;font-size:9px;font-weight:800;text-decoration:none;border:none;background:#4285f4;cursor:pointer;">Maps</button>
        </div>
      </div>`).join('')}</div>
    </div>
  </div>`;
