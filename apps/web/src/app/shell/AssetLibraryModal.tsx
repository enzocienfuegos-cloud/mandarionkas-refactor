...
export function AssetLibraryModal({ onClose }: AssetLibraryModalProps): JSX.Element {
  ...
  async function handleMoveSelectedToActiveFolder(): Promise<void> {
    ...
    setFolderError('');
    try {
      const targetFolderId = getUploadFolderId();
      for (const assetId of selectedAssetIds) {
        await moveAsset(assetId, targetFolderId);
      }
      assetController.refreshAssets();
      setSelectedAssetIds([]);
    } catch (error) {
            ...
            <div className="asset-library-browser-toolbar">
              ...
              <div className="asset-library-browser-actions">
                ...
                <button
                  className="ghost compact-action"
                  type="button"
                  disabled={selectedAssetIds.length === 0 || folderBusy}
                  onClick={() => void handleMoveSelectedToActiveFolder()}
                  title={activeFolder ? `Move selected assets to ${activeFolder.name}` : 'Move selected assets to root'}
                >
                  {activeFolder ? 'Move selected here' : 'Move selected to root'}
                </button>
                {assetController.selectedWidgetAcceptsAsset ? (
                  <button
                    className="ghost compact-action"
