...
function asDelta(value: string) {
  return value;
}

export const kpisByMode: Record<ReportingMode, ReportingKpi[]> = {
  all: [
    { id: 'impressions', label: 'Impressions', value: '8.4M', delta: asDelta('+12.4%'), direction: 'up', comparisonLabel: 'vs previous period', tone: 'fuchsia', icon: 'impressions', sparkline: makeSparkline([22, 26, 29, 31, 36, 39, 42]) },
    { id: 'clicks', label: 'Clicks', value: '148K', delta: asDelta('+8.1%'), direction: 'up', comparisonLabel: 'click volume', tone: 'fuchsia', icon: 'clicks', sparkline: makeSparkline([12, 13, 14, 14, 16, 17, 18]) },
    { id: 'ctr', label: 'CTR', value: '1.76%', delta: asDelta('+0.08%'), direction: 'up', comparisonLabel: 'engagement rate', tone: 'violet', icon: 'ctr', sparkline: makeSparkline([1.4, 1.5, 1.45, 1.52, 1.6, 1.68, 1.76]) },
    { id: 'viewability', label: 'Viewability', value: '71.4%', delta: asDelta('+3.2%'), direction: 'up', comparisonLabel: 'measured inventory', tone: 'blue', icon: 'viewability', sparkline: makeSparkline([61, 63, 64, 66, 68, 70, 71]) },
    { id: 'vcr', label: 'Video Completion Rate', value: '31.2%', delta: asDelta('+1.5%'), direction: 'up', comparisonLabel: 'video funnel', tone: 'blue', icon: 'video', sparkline: makeSparkline([26, 27, 27, 28, 29, 30, 31]) },
    { id: 'match', label: 'Identity Match Rate', value: '48.0%', delta: asDelta('+2.0%'), direction: 'up', comparisonLabel: 'resolved identities', tone: 'emerald', icon: 'identity', sparkline: makeSparkline([39, 41, 42, 43, 45, 46, 48]) },
    { id: 'attention', label: 'Engagement Attention', value: '18.4s', delta: asDelta('+7.8%'), direction: 'up', comparisonLabel: 'avg active time', tone: 'amber', icon: 'attention', sparkline: makeSparkline([11, 12, 13, 14, 15, 17, 18]) },
    { id: 'campaigns', label: 'Active Campaigns', value: '24', delta: asDelta('+3'), direction: 'up', comparisonLabel: 'currently live', tone: 'slate', icon: 'campaign', sparkline: makeSparkline([17, 18, 19, 20, 22, 23, 24]) },
  ],
  display: [
    { id: 'impressions', label: 'Impressions', value: '6.1M', delta: asDelta('+10.9%'), direction: 'up', comparisonLabel: 'served display', tone: 'fuchsia', icon: 'impressions', sparkline: makeSparkline([16, 17, 19, 20, 23, 26, 28]) },
    { id: 'clicks', label: 'Clicks', value: '123K', delta: asDelta('+6.4%'), direction: 'up', comparisonLabel: 'response volume', tone: 'fuchsia', icon: 'clicks', sparkline: makeSparkline([10, 11, 12, 12, 13, 14, 15]) },
    { id: 'ctr', label: 'CTR', value: '2.01%', delta: asDelta('+0.12%'), direction: 'up', comparisonLabel: 'click efficiency', tone: 'violet', icon: 'ctr', sparkline: makeSparkline([1.7, 1.74, 1.79, 1.84, 1.9, 1.95, 2.01]) },
    { id: 'viewability', label: 'Viewability', value: '73.6%', delta: asDelta('+2.4%'), direction: 'up', comparisonLabel: 'measured rate', tone: 'blue', icon: 'viewability', sparkline: makeSparkline([65, 66, 68, 69, 71, 72, 74]) },
    { id: 'attention', label: 'Attention Time', value: '16.8s', delta: asDelta('+5.3%'), direction: 'up', comparisonLabel: 'avg active time', tone: 'amber', icon: 'attention', sparkline: makeSparkline([12, 12, 13, 14, 15, 16, 17]) },
    { id: 'engagement', label: 'Engagement Rate', value: '4.28%', delta: asDelta('+0.5%'), direction: 'up', comparisonLabel: 'rich media events', tone: 'amber', icon: 'attention', sparkline: makeSparkline([3.2, 3.4, 3.6, 3.8, 4.0, 4.1, 4.28]) },
    { id: 'activeCampaigns', label: 'Active Campaigns', value: '18', delta: asDelta('+2'), direction: 'up', comparisonLabel: 'display live', tone: 'slate', icon: 'campaign', sparkline: makeSparkline([13, 13, 14, 15, 16, 17, 18]) },
    { id: 'activeTags', label: 'Active Tags', value: '42', delta: asDelta('+5'), direction: 'up', comparisonLabel: 'firing tags', tone: 'slate', icon: 'tag', sparkline: makeSparkline([31, 31, 33, 35, 37, 39, 42]) },
  ],
  video: [
    { id: 'starts', label: 'Video Starts', value: '24.5K', delta: asDelta('+9.7%'), direction: 'up', comparisonLabel: 'started plays', tone: 'blue', icon: 'video', sparkline: makeSparkline([15, 16, 17, 19, 21, 23, 24]) },
    { id: 'q1', label: '25% Viewed', value: '17.4K', delta: asDelta('+8.9%'), direction: 'up', comparisonLabel: 'first quartile', tone: 'blue', icon: 'video', sparkline: makeSparkline([10, 11, 12, 13, 14, 16, 17]) },
    { id: 'q2', label: '50% Viewed', value: '11.2K', delta: asDelta('+7.4%'), direction: 'up', comparisonLabel: 'midpoint', tone: 'blue', icon: 'video', sparkline: makeSparkline([6, 7, 7, 8, 9, 10, 11]) },
    { id: 'q3', label: '75% Viewed', value: '7.8K', delta: asDelta('+6.1%'), direction: 'up', comparisonLabel: 'third quartile', tone: 'blue', icon: 'video', sparkline: makeSparkline([4, 5, 5, 6, 6, 7, 8]) },
    { id: 'complete', label: 'Completions', value: '7.6K', delta: asDelta('+6.0%'), direction: 'up', comparisonLabel: 'full views', tone: 'blue', icon: 'video', sparkline: makeSparkline([4, 4, 5, 5, 6, 7, 8]) },
    { id: 'completionRate', label: 'Completion Rate', value: '31.25%', delta: asDelta('+1.2%'), direction: 'up', comparisonLabel: 'completion efficiency', tone: 'blue', icon: 'video', sparkline: makeSparkline([26, 27, 28, 29, 29, 30, 31]) },
    { id: 'watch', label: 'Avg. Watch Time', value: '14.2s', delta: asDelta('+3.6%'), direction: 'up', comparisonLabel: 'per started play', tone: 'cyan', icon: 'attention', sparkline: makeSparkline([10, 10, 11, 12, 13, 14, 14]) },
    { id: 'events', label: 'Video Events', value: '61K', delta: asDelta('+11.9%'), direction: 'up', comparisonLabel: 'captured signals', tone: 'slate', icon: 'tracker', sparkline: makeSparkline([30, 32, 35, 39, 44, 53, 61]) },
  ],
  identity: [
    { id: 'reach', label: 'Identity Reach', value: '224K', delta: asDelta('+13.0%'), direction: 'up', comparisonLabel: 'resolved users', tone: 'emerald', icon: 'identity', sparkline: makeSparkline([101, 110, 128, 136, 170, 194, 224]) },
    { id: 'match', label: 'Match Rate', value: '48.0%', delta: asDelta('+2.0%'), direction: 'up', comparisonLabel: 'resolvable events', tone: 'emerald', icon: 'identity', sparkline: makeSparkline([39, 40, 41, 43, 45, 46, 48]) },
    { id: 'unique', label: 'Unique Identities', value: '224K', delta: asDelta('+12.2%'), direction: 'up', comparisonLabel: 'deduplicated ids', tone: 'emerald', icon: 'identity', sparkline: makeSparkline([105, 113, 124, 140, 172, 195, 224]) },
    { id: 'freq', label: 'Avg. Frequency', value: '5.2', delta: asDelta('-0.4'), direction: 'down', comparisonLabel: 'per identity', tone: 'slate', icon: 'dashboard', sparkline: makeSparkline([6.1, 6.0, 5.8, 5.7, 5.5, 5.3, 5.2]) },
    { id: 'clicked', label: 'Clicked Users', value: '18.6K', delta: asDelta('+4.8%'), direction: 'up', comparisonLabel: 'engaged cohort', tone: 'fuchsia', icon: 'clicks', sparkline: makeSparkline([11, 11, 12, 13, 14, 16, 18]) },
    { id: 'hf', label: 'High-Frequency Exposed', value: '11.4K', delta: asDelta('-3.1%'), direction: 'down', comparisonLabel: 'overexposed users', tone: 'amber', icon: 'attention', sparkline: makeSparkline([16, 15, 15, 14, 13, 12, 11]) },
    { id: 'attrib', label: 'Attribution Windows', value: '3', delta: asDelta('stable'), direction: 'flat', comparisonLabel: 'configured models', tone: 'slate', icon: 'tracker', sparkline: makeSparkline([3, 3, 3, 3, 3, 3, 3]) },
    { id: 'exportable', label: 'Exportable Audiences', value: '5', delta: asDelta('+1'), direction: 'up', comparisonLabel: 'activation ready', tone: 'emerald', icon: 'export', sparkline: makeSparkline([2, 2, 3, 3, 4, 4, 5]) },
  ],
};
