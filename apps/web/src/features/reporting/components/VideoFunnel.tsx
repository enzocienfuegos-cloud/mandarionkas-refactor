...
import type { VideoFunnelRow } from '../reporting.types';
import { WidgetPanel } from './WidgetPanel';

export function VideoFunnel({ rows }: { rows: VideoFunnelRow[] }) {
  return (
    <WidgetPanel title="Video completion funnel" icon="video" tone="blue">
      {rows.length ? (
        ...
        <EmptyState
          title="No video funnel yet"
          description="Quartile and completion data will populate once the selected scope records video playback."
        />
      )}
    </WidgetPanel>
  );
