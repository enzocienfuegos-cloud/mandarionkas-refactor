  ...
  return (
    <section className="section section-premium">
      ...
      <div className="field-stack">
        <div><label>Accepts source</label><input value={String(node.props.acceptsSource ?? '')} onChange={(e) => updateWidgetProps(node.id, { acceptsSource: e.target.value })} /></div>
        <div><label>On-match action id</label><input value={String(node.props.onMatchAction ?? '')} onChange={(e) => updateWidgetProps(node.id, { onMatchAction: e.target.value })} /></div>
