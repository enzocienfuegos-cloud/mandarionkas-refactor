    ...
    <section className="section section-premium">
      ...
      <div className="field-stack">
        <div>
          ...
          <select value={durationSource} onChange={(e) => updateWidgetProps(node.id, { durationSource: e.target.value })}>
            <option value="scene">Use scene duration</option>
            <option value="custom">Custom timer duration</option>
          </select>
        </div>
        {durationSource === 'custom' ? <div><label>Duration ms</label><input type="number" min={1000} max={60000} step={500} value={Number(node.props.durationMs ?? 7000)} onChange={(e) => updateWidgetProps(node.id, { durationMs: Number(e.target.value) })} /></div> : <small className="muted">This timer follows the current scene duration.</small>}
