...
export function useOverviewData({
  ...
  const handleWorkspaceChange = async (nextWorkspaceId: string) => {
    if (!nextWorkspaceId || nextWorkspaceId === activeWorkspaceId) return false;
    try {
  ...
  return {
    ...
    workspaces,
    activeWorkspaceId,
    setCampaignIdOnWorkspaceChange: handleWorkspaceChange,
    loading,
    error,
    currentStats,
    previousStats,
...
export function useAttentionItems({
  ...
  campaignBreakdown: BreakdownItem[];
  creatives: Creative[];
  creativeBreakdown: BreakdownItem[];
  previousCtr: number;
  ...
  return useMemo<AttentionItem[]>(() => {
    ...
    if (lowCtrCampaign) {
      const delta = computeDelta(toNumber(lowCtrCampaign.ctr), previousCtr).label;
      items.push({
        id: 'campaign-low-ctr',
  ...
  }, [campaignBreakdown, creatives, creativeBreakdown, previousCtr, tagBreakdown, tags]);
}
...
export function useOverviewDashboardModel({
  ...
  identitySegments: SegmentBreakdownItem[];
