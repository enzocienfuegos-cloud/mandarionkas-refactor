...
async function regenerateVideoRenditionsForVersion(client, workspaceId, version, creative) {
  ...
  await updateCreativeVersion(client, workspaceId, version.id, {
  ...
  if (creative) {
    await updateCreative(client, workspaceId, creative.id, {
  ...
  return listVideoRenditions(client, workspaceId, version.id);
}
...
export function handleCreativeRoutes(app, { requireWorkspace, pool }) {
  ...
  app.post('/v1/creative-versions/:id/video-renditions/regenerate', { preHandler: requireWorkspace }, async (req, reply) => {
    const { workspaceId } = req.authSession;
    const { id } = req.params;
    ...
    if (version.serving_format !== 'vast_video') {
      return reply.status(400).send({ error: 'Bad Request', message: 'Only VAST video versions can regenerate renditions' });
    }

    const creative = await getCreative(pool, workspaceId, version.creative_id);
