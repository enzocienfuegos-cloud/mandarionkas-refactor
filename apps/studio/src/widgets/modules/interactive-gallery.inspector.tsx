          ...
          {items.length ? items.map((item, index) => (
            ...
            >
              <div className="inspector-card-header">
                <strong className="inspector-card-title">Item {index + 1}</strong>
                <div className="inspector-card-actions">
                  <IconButton variant="ghost" size="sm" label={`Move item ${index + 1} up`} icon={<StudioIcon icon={StudioIcons.chevronUp} size={14} />} onClick={() => moveItem(index, -1)} disabled={index === 0} />
                  <IconButton variant="ghost" size="sm" label={`Move item ${index + 1} down`} icon={<StudioIcon icon={StudioIcons.chevronDown} size={14} />} onClick={() => moveItem(index, 1)} disabled={index === items.length - 1} />
                  <Button variant="danger" size="sm" onClick={() => removeItem(index)}>Remove</Button>
                </div>
              </div>
              <small className="muted">Drag to reorder</small>
              <div className="inspector-preview-frame inspector-preview-frame--gallery">
              ...
              <AssetPickerButton
                ...
                accept="image"
                assets={assets}
                onChange={(asset) => updateItem(index, { assetId: asset.id, src: asset.src })}
                onClear={() => updateItem(index, { assetId: undefined, src: '' })}
              />
              <input
                value={item.title}
