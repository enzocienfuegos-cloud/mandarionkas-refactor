...
const interactiveGalleryViewportBaseStyle: CSSProperties = {
  flex: 1,
  borderRadius: 12,
  overflow: 'hidden',
  display: 'grid',
...
const interactiveGalleryImageStyle: CSSProperties = {
  width: '100%',
  height: '100%',
  objectFit: 'cover',
  display: 'block',
...
const interactiveGalleryFallbackStyle: CSSProperties = {
  display: 'grid',
  placeItems: 'center',
  fontSize: 26,
  fontWeight: 900,
...
const interactiveGalleryCopyStyle: CSSProperties = {
  display: 'grid',
  gap: 2,
};

const interactiveGallerySubtitleStyle: CSSProperties = {
  opacity: 0.72,
};

const interactiveGalleryButtonRowStyle: CSSProperties = {
  display: 'flex',
  gap: 8,
};

const interactiveGalleryPrevButtonBaseStyle: CSSProperties = {
  flex: 1,
  borderRadius: 10,
...
const interactiveGalleryNextButtonBaseStyle: CSSProperties = {
  flex: 1,
  borderRadius: 10,
  border: 'none',
...
function buildInteractiveGalleryViewportStyle(accent: string): CSSProperties {
  return {
    ...interactiveGalleryViewportBaseStyle,
    background: `linear-gradient(135deg, ${accent}55, var(--white-a-08))`,
  };
}

function buildInteractiveGalleryPrevButtonStyle(accent: string): CSSProperties {
  return {
    ...interactiveGalleryPrevButtonBaseStyle,
    border: `1px solid ${accent}`,
  };
}

function buildInteractiveGalleryNextButtonStyle(accent: string): CSSProperties {
  return {
    ...interactiveGalleryNextButtonBaseStyle,
    background: accent,
  };
}
  ...
  return (
    <div style={moduleShell(node, ctx)}>
      ...
      <div style={moduleBody}>
        ...
        <div style={interactiveGalleryCopyStyle}>
          <strong>{activeItem?.title ?? `Item ${activeIndex + 1}`}</strong>
          {activeItem?.subtitle ? <small style={interactiveGallerySubtitleStyle}>{activeItem.subtitle}</small> : null}
        </div>

        <div style={interactiveGalleryButtonRowStyle}>
          <button
            type="button"
            onClick={(event) => {
            ...
            style={buildInteractiveGalleryPrevButtonStyle(accent)}
          >
            Prev
          </button>
          <button
            ...
            style={buildInteractiveGalleryNextButtonStyle(accent)}
          >
            Next
          </button>
        </div>
...
export function renderInteractiveGalleryStage(node: WidgetNode, ctx: RenderContext): JSX.Element {
  const collapsed = renderCollapsedIfNeeded(node, ctx);
  if (collapsed) return collapsed;
  return <InteractiveGalleryModuleRenderer node={node} ctx={ctx} />;
}
...
export function renderInteractiveGalleryExport(node: WidgetNode): string {
  ...
  const accent = escapeHtml(String(node.style.accentColor ?? 'hsl(217 33% 17%)'));
  ...
  return `<div class="widget widget-interactive-gallery" data-widget-id="${node.id}" style="${base}"><div style="font-size:12px;font-weight:800;letter-spacing:.04em;text-transform:uppercase;color:${accent};">${escapeHtml(String(node.props.title ?? node.name))}</div><div style="flex:1;min-height:0;border-radius:12px;overflow:hidden;background:hsl(0 0% 100% / 0.06);">${background}</div><div style="display:grid;gap:2px;"><strong>${escapeHtml(activeItem?.title ?? `Item ${activeIndex + 1}`)}</strong>${activeItem?.subtitle ? `<small style="opacity:.72;">${escapeHtml(activeItem.subtitle)}</small>` : ''}</div><div style="display:flex;gap:8px;"><button class="widget-cta" type="button" style="flex:1;border-radius:10px;border:1px solid ${accent};background:transparent;color:inherit;padding:8px 10px;">Prev</button><button class="widget-cta" type="button" style="flex:1;border-radius:10px;border:none;background:${accent};color:hsl(217 33% 17%);padding:8px 10px;font-weight:800;">Next</button></div></div>`;
}
