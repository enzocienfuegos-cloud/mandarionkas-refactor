import type { WidgetNode, ActionNode, StudioState } from '../../domain/document/types';
...
function mapPanelKeyToSectionKey(panelKey: WidgetInspectorPanelKey) {
  switch (panelKey) {
    case 'position-size':
    case 'text-content':
    case 'module-config':
...
function isPanelVisible(
  ...
  const sectionKey = mapPanelKeyToSectionKey(panelKey);
  if (!sectionKey || !widget || !state) return true;
  return resolveInspectorSectionVisibility(definition, sectionKey, widget, state);
}

export function getWidgetInspectorTabs(definition: WidgetDefinition, widget?: WidgetNode, state?: StudioState): WidgetInspectorTabSpec[] {
...
export function getWidgetBehaviorPanelCount(definition: WidgetDefinition, widget?: WidgetNode, state?: StudioState): number {
  return getWidgetInspectorTabs(definition, widget, state)
