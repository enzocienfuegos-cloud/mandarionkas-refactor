...
export function renderGroupExport(
  ...
  const resolvedWidgets = buildResolvedWidgetsById(state.document);
  const scratchCoverImage = buildScratchGroupCoverDataUrl({
    node,
    state,
    widgetsById: resolvedWidgets,
    rootFrame: frame,
    resolveFrame: (widget) => widget.frame,
    assetPathMap,
  });
  const base = [
    `position:absolute`,
  ...
  return `<div class="widget widget-group widget-group-scratch" data-widget-id="${node.id}" style="${base}">
    <div
      ...
      data-scratch-cover-image="${escapeHtml(scratchCoverImage)}"
