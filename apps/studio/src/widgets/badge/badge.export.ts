...
export function renderBadgeExport(node: WidgetNode): string {
  ...
  const boxShadow = shadow.enabled
    ? shadowConfigToBoxShadow(shadow)
    : String(node.style.boxShadow ?? '0 12px 24px rgba(0,0,0,0.18)');
  const base = `${getBaseWidgetStyle(node)};display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:0 14px;border-radius:${Number(node.style.borderRadius ?? 999)}px;background:${escapeHtml(String(node.style.backgroundColor ?? '#7c3aed'))};border:1px solid ${escapeHtml(String(node.style.borderColor ?? 'rgba(255,255,255,0.18)'))};box-shadow:${escapeHtml(boxShadow)};`;
