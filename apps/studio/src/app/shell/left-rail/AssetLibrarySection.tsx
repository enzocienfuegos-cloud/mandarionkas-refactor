...
export function AssetLibrarySection({ controller }: { controller: LeftRailController }): JSX.Element {
  ...
  const compatibleWithSelection = useMemo(() => {
    ...
    if (primaryWidget.type === 'scratch-reveal') return selectedAsset.kind === 'image';
  ...
  function canApplyAssetToSelection(asset: AssetRecord): boolean {
    if (!selectedWidgetAcceptsAsset || !primaryWidget) return false;
    if (primaryWidget.type === 'video-hero' || primaryWidget.type === 'interactive-video') return asset.kind === 'video';
    if (primaryWidget.type === 'image-carousel' || primaryWidget.type === 'interactive-gallery' || primaryWidget.type === 'shoppable-sidebar') return asset.kind === 'image';
    if (primaryWidget.type === 'scratch-reveal') return asset.kind === 'image';
    if (['text', 'cta', 'badge'].includes(primaryWidget.type)) return asset.kind === 'font';
    return asset.kind === 'image';
  }

  function handleAssetTileSelection(asset: AssetRecord): void {
    setSelectedAssetId(asset.id);
    if (canApplyAssetToSelection(asset)) {
      assignAsset(asset);
    }
  }
          ...
          {filteredAssets.length ? (
            <div className="asset-library-grid">
              {filteredAssets.map((asset) => {
                ...
                const canApplyToCurrentSelection = canApplyAssetToSelection(asset);
                ...
                return (
                  <div
                    ...
                    onClick={() => handleAssetTileSelection(asset)}
                    onDoubleClick={() => canApplyToCurrentSelection ? assignAsset(asset) : undefined}
                    title={
                      canApplyToCurrentSelection
                        ? ((asset.kind === 'image' || asset.kind === 'video')
                            ? 'Click to apply to the selected widget, or drag to canvas'
                            : 'Click to apply to the selected widget')
                        : ((asset.kind === 'image' || asset.kind === 'video')
                    ...
                    onKeyDown={(event) => {
                      if (event.key === 'Enter' || event.key === ' ') {
                        event.preventDefault();
                        handleAssetTileSelection(asset);
                      }
                    }}
                    onDragStart={(event) => {
