...
describe('export engine', () => {
  ...
  it('renders scratch reveal widgets as full-bleed covers by default', () => {
    const state = createInitialState();
    const sceneId = state.document.scenes[0].id;
    state.document.widgets.scratch_2 = {
      id: 'scratch_2',
      type: 'scratch-reveal',
      name: 'Scratch & Reveal',
      sceneId,
      zIndex: 1,
      frame: { x: 0, y: 0, width: 220, height: 116, rotation: 0 },
      style: { accentColor: '#f97316', color: '#ffffff', backgroundColor: '#111827', borderRadius: 18 },
      props: {
        title: 'Scratch & Reveal',
        coverLabel: 'Scratch to reveal',
        revealLabel: '20% off today',
    ...
    state.document.scenes[0].widgetIds.push('scratch_2');
    const generic = buildGenericHtml5Adapter(state);

    const html = buildChannelHtml(state, generic);

    expect(html).toContain('data-scratch-cover-blur="0"');
    expect(html).toContain('data-scratch-widget-id="scratch_2"');
    expect(html).toContain('data-scratch-cover-image="https://cdn.example.com/cover.png"');
    expect(html).toContain('data-scratch-canvas');
  });
    ...
    const brokenBundle = {
      ...
      files: bundle.files.map((file) =>
        file.path === 'index.html' && file.content
          ? { ...file, content: file.content.replace('window.ClickTag = window.ClickTag || window.clickTag || ""', 'window.__removedClickTag = ""') }
          : file,
      ),
    };
    const gam = buildGamHtml5Adapter(state);
  ...
  it('warns when clickTag channels exceed the channel size budget', () => {
    ...
    const oversizedBundle = {
      ...
      files: [
        { path: 'index.html', mime: 'text/html;charset=utf-8', content: '<script>window.ClickTag = window.ClickTag || window.clickTag || "";</script>' },
        ...
        { path: 'hero.bin', mime: 'application/octet-stream', bytes: new Uint8Array(420 * 1024) },
      ],
    };
    const gam = buildGamHtml5Adapter(state);
