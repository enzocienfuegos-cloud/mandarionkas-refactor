...
describe('architecture guardrails', () => {
  ...
  it('avoids raw title attributes on interactive editor controls', () => {
    const scopedPrefixes = ['src/app/', 'src/inspector/', 'src/timeline/', 'src/canvas/', 'src/shared/ui/'];
    const allowlistedFiles = new Set([
      'src/inspector/sections/document/VideoAnalyticsSection.tsx',
    ]);
    const titleOnInteractiveControl = /<(?:button|Button|IconButton|SurfaceButton)\b[^>]*\btitle=/m;
    const offenders = files.flatMap((file) => {
      if (!scopedPrefixes.some((prefix) => file.relativePath.startsWith(prefix))) return [];
      if (allowlistedFiles.has(file.relativePath)) return [];
      return titleOnInteractiveControl.test(file.content) ? [`${file.relativePath} uses title= on an interactive control`] : [];
    });
    expect(offenders).toEqual([]);
  });
  ...
  it('keeps imports within declared layer boundaries', () => {
    const allowedSelfImports = new Set([
      'src/App.tsx',
      'src/main.tsx',
      // Known circular dependency: actions ↔ widgets/video.
      // action-effects needs IVideoPlayer to control video via actions;
      // effect-registry registers action handlers from within the widget layer.
