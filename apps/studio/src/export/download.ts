...
import { buildExportBundle, buildExportBundleWithRemoteAssets, buildExportSizeSetBundle, buildExportSizeSetBundleWithRemoteAssets } from './bundle';
...
export function triggerExportZipBundle(state: StudioState): void {
  void prepareExportStateWithResolvedAssets(state).then((preparedState) => {
    const bundle = preparedState.document.canvasVariants.length > 1
      ? buildExportSizeSetBundle(preparedState)
      : buildExportBundle(preparedState);
    const zip = buildZipFromBundle(bundle, state.document.name || 'smx-export');
    downloadBlob(zip.filename, new Blob([zip.bytes as unknown as Uint8Array<ArrayBuffer>], { type: zip.mime }));
  });
}
...
export async function triggerExportZipBundleResolved(state: StudioState): Promise<string> {
  const preparedState = await prepareExportStateWithResolvedAssets(state);
  const bundle = preparedState.document.canvasVariants.length > 1
    ? await buildExportSizeSetBundleWithRemoteAssets(preparedState)
    : await buildExportBundleWithRemoteAssets(preparedState);
  const zip = buildZipFromBundle(bundle, state.document.name || 'smx-export');
  downloadBlob(zip.filename, new Blob([zip.bytes as unknown as Uint8Array<ArrayBuffer>], { type: zip.mime }));
  return zip.filename;
}
