  ...
  return (
    <div className="field-stack">
      ...
      <div className="field-stack">
        {Object.entries(activeRecord?.values ?? {}).map(([field, value]) => (
          <div key={field} className="fields-grid">
            ...
            <div className="align-self-end"><Button variant="ghost" size="sm" onClick={() => {
      ...
      <div className="meta-line">
        <Button size="sm" onClick={() => upsertFeedRecord(activeSource, { id: `${activeSource}_${Date.now()}`, label: `Record ${records.length + 1}`, values: { title: 'New record' } })}>Add record</Button>
        {activeRecord ? <Button variant="ghost" size="sm" onClick={() => deleteFeedRecord(activeSource, activeRecord.id)}>Delete record</Button> : null}
      </div>
    </div>
  );
}
