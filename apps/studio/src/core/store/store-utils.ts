...
export function getSelectedWidgets(state: StudioState): WidgetNode[] {
  ...
  return state.document.selection.widgetIds.map((widgetId) => widgetsById[widgetId]).filter(Boolean);
}
...
export function getWidgetWithChildren(state: StudioState, widgetId: string): WidgetNode[] {
  ...
  if (!widget) return [];
  return [widget, ...(widget.childIds ?? []).map((id) => widgetsById[id]).filter(Boolean)];
}
...
export function getSmartPlacedFrame(state: StudioState, sceneId: string, frame: WidgetFrame): WidgetFrame {
  ...
  const widgetsById = buildResolvedWidgetsById(state.document);
  const widgets = scene.widgetIds.map((id) => widgetsById[id]).filter(Boolean);
  const padding = 20;
  const step = 28;
  let next = { ...frame };
