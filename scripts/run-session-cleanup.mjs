...
if (!baseUrl || !email || !password) {
  console.error(JSON.stringify({
    ok: false,
    message: 'Missing SMOKE_BASE_URL/VITE_API_BASE_URL or smoke credentials for session cleanup script.',
  }, null, 2));
  process.exit(1);
}
...
async function main() {
  ...
  if (!login.ok) throw new Error('Admin login failed for cleanup.');

  const cleanup = await request('/admin/maintenance/cleanup-sessions', {
    method: 'POST',
    body: JSON.stringify({}),
  });
