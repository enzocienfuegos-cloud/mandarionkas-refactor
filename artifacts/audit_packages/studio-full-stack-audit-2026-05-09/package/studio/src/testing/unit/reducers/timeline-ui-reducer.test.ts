...
describe('timelineUiReducer', () => {
  ...
  it('updates the persisted preview frame context independently from preview mode', () => {
    const state = createInitialState();
    const next = timelineUiReducer(state, { type: 'SET_PREVIEW_CONTEXT', previewContext: 'pixel8' });
    expect(next.ui.previewContext).toBe('pixel8');
    expect(next.ui.previewMode).toBe(false);
  });
});
