...
const overlayConfigInputBaseStyle: CSSProperties = {
  width: '100%',
  padding: '5px 8px',
...
const overlayConfigTextareaStyle: CSSProperties = {
  ...overlayConfigInputBaseStyle,
  height: 84,
  resize: 'vertical',
  fontFamily: 'monospace',
};

const overlayConfigPanelStyle: CSSProperties = {
...
const overlayConfigPanelTitleStyle: CSSProperties = {
  fontWeight: 700,
  fontSize: '0.78rem',
  color: '#7dd3fc',
  marginBottom: 8,
...
const overlayConfigSectionStyle: CSSProperties = {
  marginBottom: 8,
};

const overlayConfigHintStyle: CSSProperties = {
  fontSize: '0.68rem',
...
function FieldInput({
  ...
  switch (field.inputType) {
    ...
    case 'number':
      return <input style={overlayConfigInputBaseStyle} type="number" min={field.min} max={field.max} value={String(value ?? field.defaultValue ?? '')} onChange={(e) => onChange(Number(e.target.value))} />;
    case 'color':
      return <input type="color" value={String(value ?? field.defaultValue ?? '#000000')} onChange={(e) => onChange(e.target.value)} />;
    case 'select':
      return (
        <select style={overlayConfigInputBaseStyle} value={String(value ?? field.defaultValue ?? '')} onChange={(e) => onChange(e.target.value)}>
          {field.options?.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
        </select>
      );
    case 'html-editor':
      return <textarea style={overlayConfigTextareaStyle} value={String(value ?? field.defaultValue ?? '')} onChange={(e) => onChange(e.target.value)} />;
    default:
      return <input style={overlayConfigInputBaseStyle} type="text" value={String(value ?? field.defaultValue ?? '')} onChange={(e) => onChange(e.target.value)} />;
  }
}
...
export function OverlayConfigForm({ overlay, onChange }: OverlayConfigFormProps): JSX.Element | null {
  ...
  return (
    <div style={overlayConfigPanelStyle}>
      <div style={overlayConfigPanelTitleStyle}>{schema.label}</div>

      <div className="fields-grid" style={overlayConfigSectionStyle}>
        <div>
      ...
      {schema.fields.map((field) => (
        <div key={field.key} style={overlayConfigSectionStyle}>
          ...
          />
          {field.hint ? <div style={overlayConfigHintStyle}>{field.hint}</div> : null}
        </div>
      ))}
    </div>
  );
