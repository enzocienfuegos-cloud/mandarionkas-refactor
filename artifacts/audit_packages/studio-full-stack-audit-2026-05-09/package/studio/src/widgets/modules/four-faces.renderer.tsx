...
function buildDotStyle(active: boolean): CSSProperties {
  return {
    ...
    height: 8,
    borderRadius: 4,
    background: active ? 'var(--surface-card-light)' : 'var(--white-a-45)',
...
function FourFacesRenderer({ node, ctx }: { node: WidgetNode; ctx: RenderContext }): JSX.Element {
  ...
  const homeTitleColor = String(node.props.homeTitleColor ?? FOUR_FACES_DEFAULT_HOME.titleColor);
  const homeSubtitle = String(node.props.homeSubtitle ?? FOUR_FACES_DEFAULT_HOME.subtitle);
  const homeSubtitleColor = String(node.props.homeSubtitleColor ?? FOUR_FACES_DEFAULT_HOME.subtitleColor);
  const homeHintText = String(node.props.homeHintText ?? FOUR_FACES_DEFAULT_HOME.hintText);
  const homeHintColor = String(node.props.homeHintColor ?? FOUR_FACES_DEFAULT_HOME.hintColor);
  ...
  const homeCtaTextColor = String(node.props.homeCtaTextColor ?? FOUR_FACES_DEFAULT_HOME.ctaTextColor);
  ...
  const shellStyle: CSSProperties = ctx.previewMode
    ? { width: '100%', height: '100%', position: 'relative', overflow: 'hidden', background: 'var(--neutral-black)', touchAction: 'none', userSelect: 'none', cursor: dismissed ? 'default' : 'grab' }
