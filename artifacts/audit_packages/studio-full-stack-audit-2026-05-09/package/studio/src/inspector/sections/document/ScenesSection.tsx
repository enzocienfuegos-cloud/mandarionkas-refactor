  ...
  return (
    <div className="field-stack">
      ...
      <div className="meta-line">
        <Button size="sm" onClick={() => updateScene(activeScene.id, { flow: { ...(activeScene.flow ?? {}), branches: [...branches, { source: 'custom', field: 'segment', value: 'a', targetSceneId: scenes.find((scene) => scene.id !== activeScene.id)?.id ?? activeScene.id, label: `Branch ${branches.length + 1}` }] } })}>Add branch</Button>
      </div>
    </div>
  );
}
