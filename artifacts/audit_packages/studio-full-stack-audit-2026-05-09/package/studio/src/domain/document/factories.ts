...
import { createCanvasVariantFromCanvas, syncDocumentCanvasToVariant } from './canvas-variants';
...
export function createEmptyDocument(options: InitialDocumentOptions = {}): StudioDocument {
  ...
  const canvas = createCanvasSeed(options);
  const masterVariant = createCanvasVariantFromCanvas(canvas, {
  ...
  return {
    ...
    version: 1,
    canvas,
    canvasVariants: [masterVariant],
    activeCanvasVariantId: masterVariant.id,
    widgetOverrides: {},
    sharedLayers: {},
    scenes: [scene],
    widgets: {},
