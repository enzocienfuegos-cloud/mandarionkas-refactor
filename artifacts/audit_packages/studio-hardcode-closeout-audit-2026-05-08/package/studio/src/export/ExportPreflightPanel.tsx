...
function buildPillBorderStyle(color: string): React.CSSProperties {
  return { borderColor: color };
}

function renderChannelIssue(issue: ChannelRequirement, index: number, color: string): JSX.Element {
  return (
    <div key={`${issue.id}-${index}`} className="pill" style={buildPillBorderStyle(color)}>
      {issue.label}
    </div>
  );
}
...
function renderPackageIssue(issue: ExportPackageComplianceIssue, index: number, color: string): JSX.Element {
  return (
    <div key={`${issue.code}-${issue.targetId ?? index}`} className="pill" style={buildPillBorderStyle(color)}>
  ...
  return (
    <div className={`export-preflight-panel ${compact ? 'export-preflight-panel--compact' : ''}`}>
      ...
      {preflight.summary.blockers === 0 ? (
        <div className="pill" style={buildPillBorderStyle('rgba(34,197,94,.35)')}>
          Export available · run the resolved export
        </div>
      ) : null}
      {preflight.summary.topBlocker ? (
        <div className="pill" style={buildPillBorderStyle('rgba(239,68,68,.45)')}>
          blocker · {preflight.summary.topBlocker}
        </div>
      ) : null}
      {preflight.summary.topWarning ? (
        <div className="pill" style={buildPillBorderStyle('rgba(245,158,11,.45)')}>
