...
function buildSpeedTestGaugeFrameStyle(gaugeHeight: number, isFast: boolean): CSSProperties {
  return {
    ...
    height: gaugeHeight,
    background: isFast
      ? 'radial-gradient(circle at 50% 100%, rgba(34,197,94,.20), rgba(15,23,42,0) 68%)'
      : 'radial-gradient(circle at 50% 100%, rgba(45,212,191,.28), rgba(15,23,42,0) 68%)',
  };
}
...
function buildSpeedTestGaugeBorderStyle(compact: boolean, gaugeBorder: number, isFast: boolean): CSSProperties {
  return {
    ...
    border: `${gaugeBorder}px solid ${isFast ? 'rgba(120,255,196,.24)' : 'rgba(255,255,255,.08)'}`,
    borderBottom: 'none',
    transform: 'scaleX(.92)',
  };
}
