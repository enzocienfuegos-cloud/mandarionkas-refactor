...
export function useWidgetBehaviorActions() {
  return useMemo(() => ({
    addWidgetAction: (widgetId: string, initialPatch?: Partial<ActionNode>) => dispatch({ type: 'ADD_WIDGET_ACTION', widgetId, initialPatch }),
    setActiveAction: (actionId?: string) => dispatch({ type: 'SET_ACTIVE_ACTION', actionId }),
