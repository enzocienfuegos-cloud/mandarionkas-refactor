...
function renderBadgeExport(node: import('../../domain/document/types').WidgetNode): string {
  ...
  return `<div class="widget widget-badge" data-widget-id="${node.id}" style="${base}">${icon ? `<span aria-hidden="true">${escapeHtml(icon)}</span>` : ''}<span data-text-slot="primary">${text}</span></div>`;
}
