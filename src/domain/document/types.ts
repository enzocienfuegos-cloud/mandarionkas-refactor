...
export type ActionNode = { id: string; widgetId: string; trigger: ActionTrigger; type: ActionType; targetWidgetId?: string; targetSceneId?: string; targetKey?: string; url?: string; text?: string; label?: string; };
...
export type StudioState = { document: StudioDocument; ui: { zoom: number; playheadMs: number; isPlaying: boolean; previewMode: boolean; hoveredWidgetId?: string; activeWidgetId?: string; activeActionId?: string; lastTriggeredActionLabel?: string; activeVariant: VariantName; activeFeedSource: BindingSource; activeFeedRecordId: string; activeProjectId?: string; activeLeftTab: 'widgets' | 'layers' | 'assets' | 'flow'; stageBackdrop: 'dark' | 'gray' | 'light'; showStageRulers: boolean; }; };
