...
export function timelineUiReducer(state: StudioState, command: StudioCommand): StudioState {
  switch (command.type) {
    ...
    case 'SET_ACTIVE_ACTION':
      return { ...state, ui: { ...state.ui, activeActionId: command.actionId } };
    case 'ADD_KEYFRAME': {
      const widget = state.document.widgets[command.widgetId];
      const scene = currentScene(state);
      if (!widget || !scene) return state;
    ...
    case 'ADD_WIDGET_ACTION': {
      ...
      const action: ActionNode = {
        ...
        trigger: command.trigger ?? 'click',
        type: command.actionType ?? 'open-url',
        label: 'Action',
        ...(command.initialPatch ?? {}),
      };
      return withDirty({
        ...state,
        document: { ...state.document, actions: { ...state.document.actions, [actionId]: action } },
        ui: { ...state.ui, activeActionId: actionId },
      });
    }
    ...
    case 'REMOVE_WIDGET_ACTION': {
      ...
      return withDirty({
        ...state,
        document: { ...state.document, actions },
        ui: { ...state.ui, activeActionId: state.ui.activeActionId === target.id ? undefined : state.ui.activeActionId },
      });
    }
    case 'EXECUTE_ACTION': {
